...
def collate_fn_bbox_ixywhc(batch):
    ...
    targets = [np.concatenate([b*np.ones((len(_id), 1)), x1y1wh_to_xywh(_x1y1wh), np.array(_id)[:, np.newaxis]], axis = 1) \
                               for b, _x1y1wh, _id in zip(range(len(batch)), x1y1wh_trans, id_trans)]
